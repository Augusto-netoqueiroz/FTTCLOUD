...
<div 
    x-data="usersPage({ initialUsers: <?php echo \Illuminate\Support\Js::from($users)->toHtml() ?>, endpoints: <?php echo \Illuminate\Support\Js::from($endpoints)->toHtml() ?> })" 
    class="p-6"
>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('usersPage', ({ initialUsers, endpoints }) => ({
        ...
        async submitCreate(){
            this.errorsCreate = {};
            try{
                const res = await fetch(`<?php echo e(route('users.store')); ?>`, {
                    method: 'POST',
                    headers: {
        ...
        async submitEdit(){
            ...
            this.errorsEdit = {};
            try{
                const res = await fetch(`<?php echo e(url('/users')); ?>/${this.current.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': this.csrf(),
        ...
        async destroy(u){
            ...
            try{
                const res = await fetch(`<?php echo e(url('/users')); ?>/${u.id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': this.csrf(),
                        'Accept': 'application/json',
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/fttelecom/resources/views/users/index.blade.php ENDPATH**/ ?>